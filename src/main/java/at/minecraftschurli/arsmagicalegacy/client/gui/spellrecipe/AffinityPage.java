...
class AffinityPage extends Page<Pair<Holder<Affinity>, Double>> {
    ...
    @Override
    public void extractElement(Pair<Holder<Affinity>, Double> element, int index, GuiGraphicsExtractor guiGraphics, int x, int y) {
        ...
        guiGraphics.text(AMClientUtil.font(), "%.3f".formatted(element.getSecond()), x + size + spacing, y + 4 + index * (size + spacing), element.getFirst().value().color(), false);
    }
