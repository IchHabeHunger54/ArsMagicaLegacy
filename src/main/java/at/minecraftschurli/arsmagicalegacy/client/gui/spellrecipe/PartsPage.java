...
class PartsPage extends Page<Holder<SpellPart>> {
    ...
    @Override
    public void renderElement(Holder<SpellPart> element, int index, GuiGraphicsExtractor guiGraphics, int x, int y) {
    ...
    private static Holder<Skill> skill(Holder<SpellPart> spellPart) {
        return AMRegistries.skills(true).getHolder(spellPart.getKey().identifier()).get();
    }
}
