...
public interface AMBlocks {
    ...
    // @formatter:off
    DeferredBlock<AirBlock>                    SPELL_LIGHT                 = register("spell_light",                 AirBlock::new, copyProperties(Blocks.AIR).lightLevel(_ -> 15));
    DeferredBlock<SpellRuneBlock>              SPELL_RUNE                  = register("spell_rune",                  SpellRuneBlock::new, copyProperties(Blocks.AIR).noOcclusion());
    DeferredBlock<LiquidBlock>                 LIQUID_ETHERIUM             = register("liquid_etherium",             p -> new LiquidBlock(AMFluids.LIQUID_ETHERIUM.get(), p), properties().replaceable().noCollission().strength(100).pushReaction(PushReaction.DESTROY).noLootTable().liquid().sound(SoundType.EMPTY).lightLevel(_ -> 5));
    DeferredBlock<LiquidEtheriumCauldronBlock> LIQUID_ETHERIUM_CAULDRON    = register("liquid_etherium_cauldron",    LiquidEtheriumCauldronBlock::new, copyProperties(Blocks.CAULDRON).lightLevel(_ -> 5));
    DeferredBlock<OcculusBlock>                OCCULUS                     = register("occulus",                     OcculusBlock::new, properties().strength(3, 5));
    DeferredBlock<InscriptionTableBlock>       INSCRIPTION_TABLE           = register("inscription_table",           InscriptionTableBlock::new, properties().strength(2).lightLevel(_ -> 1).noOcclusion());
    ...
    DeferredBlock<TransparentBlock>            MAGIC_WALL                  = register("magic_wall",                  TransparentBlock::new, properties().strength(3).noOcclusion().isValidSpawn(Blocks::never).isRedstoneConductor((_, _, _) -> false).isSuffocating((_, _, _) -> false).isViewBlocking((_, _, _) -> false));
    ...
    DeferredBlock<CelestialPrismBlock>         CELESTIAL_PRISM             = register("celestial_prism",             CelestialPrismBlock::new, properties().requiresCorrectToolForDrops().strength(1.5f, 6).noOcclusion().lightLevel(_ -> 1).emissiveRendering((_, _, _) -> true));
    DeferredBlock<BlackAuremBlock>             BLACK_AUREM                 = register("black_aurem",                 BlackAuremBlock::new, properties().mapColor(MapColor.COLOR_RED).noOcclusion().noCollission().lightLevel(_ -> 2));
